...
async function getBranch(dest: string) {
  try { await asyncExec(`git -C "${dest}" rev-parse --verify main`); return "main"; } catch {}
  try { await asyncExec(`git -C "${dest}" rev-parse --verify master`); return "master"; } catch {}
  return "main";
}

async function cloneRepo(url: string, dest: string) {
  await asyncExec(`git clone "${url}" "${dest}"`);
  const branch = await getBranch(dest);
  await asyncExec(`git -C "${dest}" checkout ${branch}`);
  // Remove the remote so it's a standalone archive (embedded repo, no online reference)
  await asyncExec(`git -C "${dest}" remote remove origin`);
}
...
function TextInput({ onSubmit, placeholder = "" }: { onSubmit: (val: string) => void, placeholder?: string }) {
  ...
  return <Text color="cyan">{value}{value.length === 0 && placeholder ? <Text color="gray">{placeholder}</Text> : null}</Text>;
}

function App() {
  const { exit } = useApp();
  const [step, setStep] = useState<'username' | 'fetching' | 'processing' | 'finishing' | 'done'>('username');
  ...
  const addLog = useCallback((msg: string) => {
    ...
    logTimer.current = setTimeout(() => setLog(null), 3000);
  }, []);

  useEffect(() => {
    loadStore().then(s => { storeRef.current = s; });
  }, []);
  ...
  useEffect(() => {
    if (step === 'fetching' && username) {
      (async () => {
        try {
          const { stdout } = await asyncExec(`gh repo list ${username} --json name,description,createdAt,url --limit 500`);
          const list = JSON.parse(stdout) ?? [];
          list.sort((a: any, b: any) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
          
          // Filter out repos that are already in the store
          const existingNames = new Set(storeRef.current.repos.map((r: any) => r.name));
  ...
  useEffect(() => {
    if (step === 'processing' && index >= repos.length && activeTasks.size === 0) {
      setStep('finishing');
    }
  ...
  useInput((input, _) => {
    ...
    if (input === 'a') {
      ...
      const dest = path.join(folder, repo.name);
      ...
      (async () => {
        try {
          ...
          storeRef.current.repos.push({
            ...
            archivedPath: path.relative(process.cwd(), dest),
  ...
  return (
    <Box flexDirection="column" padding={1}>
      <Text color="magenta" bold>repo-archiver</Text>
      <Newline />
      
      {step === 'username' && (
        <Box>
          <Text>GitHub username: </Text>
          <TextInput onSubmit={(val) => { setUsername(val); setStep('fetching'); }} placeholder="nebula-developer" />
        </Box>
      )}

      {step === 'fetching' && <Text color="yellow">Fetching repositories...</Text>}
        ...
        <>
          <Box flexDirection="column" marginBottom={1}>
            ...
            {Array.from(activeTasks.entries()).map(([name, status]) => (
              <Text key={name}>  {color.cyan(name)}: {status}</Text>
            ))}
            {activeTasks.size === 0 && <Text color="gray">  No active tasks</Text>}
          </Box>
          ...
          {step === 'processing' && index < repos.length && (
            <Box flexDirection="column" borderStyle="round" borderColor="blue" padding={1}>
              ...
              <Newline />
              <Text>Action? ({color.bold('a')}rchive / {color.bold('i')}gnore / {color.bold('d')}elete / {color.bold('q')}uit)</Text>
            </Box>
          )}

          {step === 'finishing' && <Text color="green">Finishing up...</Text>}
          {step === 'done' && <Text color="green" bold>All done!</Text>}
